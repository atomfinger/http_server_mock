...
-spec be_ok({ok, EUK} | {error, any()}) -> EUK.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, EUP}) -> EUP.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(EUS)) -> EUS.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
