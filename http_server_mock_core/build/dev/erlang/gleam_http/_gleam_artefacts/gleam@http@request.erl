...
-spec set_header(request(DXM), binary(), binary()) -> request(DXM).
...
-spec prepend_header(request(DXP), binary(), binary()) -> request(DXP).
...
-spec set_body(request(any()), DXU) -> request(DXU).
set_body(Req, Body) ->
    {request,
...
-spec map(request(DXW), fun((DXW) -> DXY)) -> request(DXY).
...
-spec set_query(request(DYI), list({binary(), binary()})) -> request(DYI).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(DYM), gleam@http:method()) -> request(DYM).
...
-spec set_scheme(request(DYT), gleam@http:scheme()) -> request(DYT).
...
-spec set_host(request(DYW), binary()) -> request(DYW).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(DYZ), integer()) -> request(DYZ).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(DZC), binary()) -> request(DZC).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(DZF), binary(), binary()) -> request(DZF).
...
-spec remove_cookie(request(DZL), binary()) -> request(DZL).
