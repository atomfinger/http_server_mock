...
-type response(EHG) :: {response, integer(), list({binary(), binary()}), EHG}.

-file("src/gleam/http/response.gleam", 87).
?DOC(" Set the body of the response, overwriting any existing body.\n").
-spec set_body(response(any()), EID) -> response(EID).
...
-spec try_map(response(EHH), fun((EHH) -> {ok, EHJ} | {error, EHK})) -> {ok,
        response(EHJ)} |
    {error, EHK}.
...
-spec set_header(response(EHV), binary(), binary()) -> response(EHV).
...
-spec prepend_header(response(EHY), binary(), binary()) -> response(EHY).
...
-spec map(response(EIF), fun((EIF) -> EIH)) -> response(EIH).
...
-spec expire_cookie(response(EIQ), binary(), gleam@http@cookie:attributes()) -> response(EIQ).
