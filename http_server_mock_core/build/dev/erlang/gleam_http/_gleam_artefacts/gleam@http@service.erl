...
-spec map_response_body(
    fun((ELC) -> gleam@http@response:response(ELD)),
    fun((ELD) -> ELF)
) -> fun((ELC) -> gleam@http@response:response(ELF)).
...
-spec prepend_response_header(
    fun((ELH) -> gleam@http@response:response(ELI)),
    binary(),
    binary()
) -> fun((ELH) -> gleam@http@response:response(ELI)).
...
-spec ensure_post(gleam@http@request:request(ELL)) -> {ok,
        gleam@http@request:request(ELL)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(ELU)) -> ELW)) -> fun((gleam@http@request:request(ELU)) -> ELW).
