...
-opaque stub_builder(FSE, FSF) :: {stub_builder,
...
-spec matching(
    stub_builder(any(), FSJ),
    http_server_mock@types:request_matcher()
) -> stub_builder(with_matcher(), FSJ).
...
-spec responding_with(
    stub_builder(FSO, any()),
    http_server_mock@types:response_definition()
) -> stub_builder(FSO, with_response()).
...
-spec with_id(stub_builder(FSU, FSV), binary()) -> stub_builder(FSU, FSV).
with_id(Builder, Id) ->
    {stub_builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_priority(stub_builder(FTA, FTB), integer()) -> stub_builder(FTA, FTB).
...
-spec in_scenario(stub_builder(FTG, FTH), binary()) -> stub_builder(FTG, FTH).
...
-spec when_state_is(stub_builder(FTM, FTN), binary()) -> stub_builder(FTM, FTN).
...
-spec then_transition_to(stub_builder(FTS, FTT), binary()) -> stub_builder(FTS, FTT).
