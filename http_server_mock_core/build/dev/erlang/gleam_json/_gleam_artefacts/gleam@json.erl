...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EOM)) -> {ok, EOM} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EOG)) -> {ok, EOG} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EOC)) -> {ok, EOC} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EOS), fun((EOS) -> json())) -> json().
...
-spec array(list(EOW), fun((EOW) -> json())) -> json().
