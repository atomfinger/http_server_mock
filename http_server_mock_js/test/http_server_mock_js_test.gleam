...
fn raw_get(url: String) -> #(Int, String)

fn get(url: String) -> TestResponse {
  let #(status, body) = raw_get(url)
  TestResponse(status: status, body: body)
}

@external(javascript, "./usage_test_ffi.mjs", "syncPost")
fn raw_post(url: String, body: String, content_type: String) -> #(Int, String)

fn post(url: String, body: String, content_type: String) -> TestResponse {
  let #(status, response_body) = raw_post(url, body, content_type)
  TestResponse(status: status, body: response_body)
}
