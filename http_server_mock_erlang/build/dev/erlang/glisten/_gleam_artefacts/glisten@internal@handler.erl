...
-type message(HJL) :: {internal, internal_message()} | {user, HJL}.

-type loop_message(HJM) :: {packet, bitstring()} | {custom, HJM}.

-type loop_state(HJN, HJO) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(HJO)),
        glisten@transport:transport(),
        HJN}.

-type connection(HJP) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(HJP))}.

-type next(HJQ, HJR) :: {continue,
        HJQ,
        gleam@option:option(gleam@erlang@process:selector(HJR))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(HJS, HJT) :: {handler,
        glisten@socket:socket(),
        fun((HJS, loop_message(HJT), connection(HJT)) -> next(HJS, loop_message(HJT))),
        fun((connection(HJT)) -> {HJS,
            gleam@option:option(gleam@erlang@process:selector(HJT))}),
        gleam@option:option(fun((HJS) -> nil)),
        glisten@transport:transport(),
        glisten@socket@options:active_state()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(HKG) -> next(HKG, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(HKK, HKL), gleam@erlang@process:selector(HKL)) -> next(HKK, HKL).
...
?DOC(false).
-spec start(handler(any(), HLA)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(HLA)))} |
