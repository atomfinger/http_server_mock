...
-type message(IGZ) :: {packet, bitstring()} | {user, IGZ}.
...
-type connection(IHA) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(IHA))}.

-opaque next(IHB, IHC) :: {continue,
        IHB,
        gleam@option:option(gleam@erlang@process:selector(IHC))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(IHD, IHE) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(IHE)) -> {IHD,
            gleam@option:option(gleam@erlang@process:selector(IHE))}),
        fun((IHD, message(IHE), connection(IHE)) -> next(IHD, message(IHE))),
        gleam@option:option(fun((IHD) -> nil)),
        integer(),
        boolean(),
        boolean(),
        ...
        gleam@option:option(gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(IHE))))),
...
-spec continue(IIE) -> next(IIE, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 152).
-spec with_selector(next(III, IIJ), gleam@erlang@process:selector(IIJ)) -> next(III, IIJ).
...
?DOC(false).
-spec convert_next(next(IIX, IIY)) -> glisten@internal@handler:next(IIX, IIY).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(IJD, IJE), fun((IJE) -> IJH)) -> next(IJD, IJH).
...
-spec map_user_selector(gleam@erlang@process:selector(message(IJK))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(IJK)).
...
-spec convert_loop(
    fun((IJP, message(IJQ), connection(IJQ)) -> next(IJP, message(IJQ)))
) -> fun((IJP, glisten@internal@handler:loop_message(IJQ), glisten@internal@handler:connection(IJQ)) -> glisten@internal@handler:next(IJP, glisten@internal@handler:loop_message(IJQ))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(IJV)) -> {IJX,
        gleam@option:option(gleam@erlang@process:selector(IJV))})
) -> fun((glisten@internal@handler:connection(IJV)) -> {IJX,
    gleam@option:option(gleam@erlang@process:selector(IJV))}).
...
-spec new(
    fun((connection(IKD)) -> {IKF,
        gleam@option:option(gleam@erlang@process:selector(IKD))}),
    fun((IKF, message(IKD), connection(IKD)) -> next(IKF, message(IKD)))
) -> builder(IKF, IKD).
new(On_init, Loop) ->
    {builder,
        loopback,
...
-spec with_close(builder(IKM, IKN), fun((IKM) -> nil)) -> builder(IKM, IKN).
...
-spec with_pool_size(builder(IKS, IKT), integer()) -> builder(IKS, IKT).
...
?DOC(false).
-spec with_http2(builder(IKY, IKZ)) -> builder(IKY, IKZ).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec bind(builder(ILE, ILF), binary()) -> builder(ILE, ILF).
...
-spec with_ipv6(builder(ILK, ILL)) -> builder(ILK, ILL).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(ILQ, ILR), binary(), binary()) -> builder(ILQ, ILR).
...
-spec with_connection_factory_name(
    builder(IMJ, IMK),
    gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(IMK))))
) -> builder(IMJ, IMK).
