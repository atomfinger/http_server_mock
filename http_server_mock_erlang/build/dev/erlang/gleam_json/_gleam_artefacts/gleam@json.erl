...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FFI)) -> {ok, FFI} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FFC)) -> {ok, FFC} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FEY)) -> {ok, FEY} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
