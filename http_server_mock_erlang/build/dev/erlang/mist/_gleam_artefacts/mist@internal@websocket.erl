...
-type valid_message(NCO) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, NCO}.

-type websocket_message(NCP) :: {valid, valid_message(NCP)} | invalid.
...
-type handler_message(NCQ) :: {internal, gramps@websocket:frame()} | {user, NCQ}.

-type websocket_state(NCR) :: {websocket_state,
        bitstring(),
        NCR,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(NDV))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(NDV))).
...
-spec apply_frames(
    ...
    fun((NDL, handler_message(NDM), websocket_connection()) -> mist@internal@next:next(NDL, NDM)),
    websocket_connection(),
    mist@internal@next:next(NDL, websocket_message(NDM)),
    fun((NDL) -> nil)
) -> mist@internal@next:next(NDL, websocket_message(NDM)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {NDA,
        gleam@option:option(gleam@erlang@process:selector(NDB))}),
    fun((NDA) -> nil),
    fun((NDA, handler_message(NDB), websocket_connection()) -> mist@internal@next:next(NDA, NDB)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
