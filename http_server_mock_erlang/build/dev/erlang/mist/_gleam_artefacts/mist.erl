...
-opaque next(NQO, NQP) :: {continue,
        NQO,
        gleam@option:option(gleam@erlang@process:selector(NQP))} |
    normal_stop |
...
-opaque builder(NQQ, NQR) :: {builder,
        integer(),
        fun((gleam@http@request:request(NQQ)) -> gleam@http@response:response(NQR)),
...
-type websocket_message(NQS) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, NQS}.
...
-type chunk_next(NQT) :: {chunk_continue, NQT} |
    chunk_stop |
    {chunk_abort, binary()}.

-file("src/mist.gleam", 53).
-spec continue(NQU) -> next(NQU, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 57).
-spec with_selector(next(NQY, NQZ), gleam@erlang@process:selector(NQZ)) -> next(NQY, NQZ).
...
-spec convert_next(next(NRN, NRO)) -> mist@internal@next:next(NRN, NRO).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(NSP)) -> gleam@http@response:response(NSR))
) -> builder(NSP, NSR).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(NSV, NSW), integer()) -> builder(NSV, NSW).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), NTB),
    integer(),
    gleam@http@response:response(NTB)
) -> builder(mist@internal@http:connection(), NTB).
...
-spec bind(builder(NTN, NTO), binary()) -> builder(NTN, NTO).
...
-spec with_ipv6(builder(NTT, NTU)) -> builder(NTT, NTU).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(NTZ, NUA), binary(), binary()) -> builder(NTZ, NUA).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(NUP)
) -> {ok, websocket_message(NUP)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((NUV, websocket_message(NUW), mist@internal@websocket:websocket_connection()) -> next(NUV, NUW)),
    fun((mist@internal@websocket:websocket_connection()) -> {NUV,
        gleam@option:option(gleam@erlang@process:selector(NUW))}),
    fun((NUV) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(NVK)) -> NVM),
    fun((NVM, NVK, s_s_e_connection()) -> gleam@otp@actor:next(NVM, NVK))
) -> gleam@http@response:response(response_data()).
...
-spec chunked(
    gleam@http@request:request(mist@internal@http:connection()),
    gleam@http@response:response(any()),
    fun((gleam@erlang@process:subject(NVV)) -> NVX),
    fun((NVX, NVV, mist@internal@http:connection()) -> chunk_next(NVX))
) -> gleam@http@response:response(response_data()).
...
-spec chunk_continue(NWC) -> chunk_next(NWC).
