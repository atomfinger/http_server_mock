...
-type message(FHQ) :: {message, FHQ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FHR, FHS) :: {continue,
        FHR,
        gleam@option:option(gleam@erlang@process:selector(FHS))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FHT, FHU) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FHT,
        gleam@erlang@process:selector(message(FHU)),
        gleam@otp@system:debug_state(),
        fun((FHT, FHU) -> next(FHT, FHU))}.

-type started(FHV) :: {started, gleam@erlang@process:pid_(), FHV}.

-opaque initialised(FHW, FHX, FHY) :: {initialised,
        FHW,
        gleam@option:option(gleam@erlang@process:selector(FHX)),
        FHY}.

-opaque builder(FHZ, FIA, FIB) :: {builder,
        fun((gleam@erlang@process:subject(FIA)) -> {ok,
                initialised(FHZ, FIA, FIB)} |
            {error, binary()}),
        integer(),
        fun((FHZ, FIA) -> next(FHZ, FIA)),
        gleam@option:option(gleam@erlang@process:name(FIA))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FIC) :: {ack, {ok, FIC} | {error, binary()}} |
...
-spec selecting(
    initialised(FJF, any(), FJH),
    gleam@erlang@process:selector(FJL)
) -> initialised(FJF, FJL, FJH).
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FKG)) -> {ok, initialised(FKI, FKG, FKJ)} |
        {error, binary()})
) -> builder(FKI, FKG, FKJ).
...
-spec initialise_actor(
    builder(any(), any(), FML),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, FML} | {error, binary()})
...
-spec start(builder(any(), any(), FMY)) -> {ok, started(FMY)} |
    {error, start_error()}.
start(Builder) ->
...
-spec call(
    gleam@erlang@process:subject(FNH),
    integer(),
    fun((gleam@erlang@process:subject(FNJ)) -> FNH)
) -> FNJ.
