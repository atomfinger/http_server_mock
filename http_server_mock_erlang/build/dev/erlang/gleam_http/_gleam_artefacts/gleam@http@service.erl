...
-spec map_response_body(
    fun((EYF) -> gleam@http@response:response(EYG)),
    fun((EYG) -> EYI)
) -> fun((EYF) -> gleam@http@response:response(EYI)).
...
-spec prepend_response_header(
    fun((EYK) -> gleam@http@response:response(EYL)),
    binary(),
    binary()
) -> fun((EYK) -> gleam@http@response:response(EYL)).
...
-spec ensure_post(gleam@http@request:request(EYO)) -> {ok,
        gleam@http@request:request(EYO)} |
    {error, nil}.
ensure_post(Req) ->
